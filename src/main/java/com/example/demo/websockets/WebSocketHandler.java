...
/**
 * WebSocketHandler is responsible for handling WebSocket sessions and providing
 * methods to send specific messages to the client through WebSocket. This
 * handler enables the server to communicate with the client in real time by
 * sending signals or updates as needed.
 *
 * <p>
 * This class includes methods for establishing WebSocket connections, managing
 * sessions, and sending messages to update the frontend UI, such as reloading
 * the interface or updating the chess game elements (clocks, move list, etc.).
 */
@Component
public class WebSocketHandler extends TextWebSocketHandler {
	 ...
	 * <p>
	 * Declared as static to ensure that only one active session is maintained at a
	 * time, enabling consistent handling of WebSocket messages and connection
	 * status.
	 * </p>
	 */
	...
	/**
	 * Invoked after a WebSocket connection is established. Sets the session and
	 * logs the session ID.
	 *
	...
	/**
	 * Sends a reload signal to the client through WebSocket to trigger an interface
	 * refresh. Verifies that the session is open before sending the message.
	 *
	...
	/**
	 * Sends a custom message to the client. Prepends the message type to the
	 * message content.
	 *
	...
	/**
	 * Sends a message to update the clocks in the client UI. Checks if the session
	 * is open.
	 *
	 * @throws Exception if sending the message fails
	 */
