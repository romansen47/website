 ...
 * application. It defines functionality for move validation, promotion handling,
 * and interaction with the evaluation engine, as well as utility methods for
 * UI and gameplay state management.
 *
 * Key responsibilities of this interface include:
 * - Retrieving possible moves, source fields, and target fields based on the current
 *   game state to facilitate move validation and availability display.
 * - Managing valid promotions for a given piece to ensure valid move options.
 * - Calculating ratios for evaluation bars to visually display engine evaluations.
 * - Converting screen coordinates to chessboard positions for accurate interaction mapping.
 * - Managing communication with the evaluation engine to fetch move evaluations and
 *   remove duplicates from move lists.
 * - Checking the game state against expected outcomes and controlling access for
 *   human interactions based on engine activity.
 * - Sending signals to reload or reset the application as needed.
 *
...
public interface ApiControllerHelper {
    ...
    /**
     * Retrieves the source fields that a piece can be moved from in the current game state.
     *
    ...
    /**
     * Retrieves the target fields that a piece can be moved to in the current game state.
     *
    ...
    /**
     * Sets the valid promotion moves for a piece, based on available moves and the selected move.
     *
    ...
    /**
     * Retrieves the list of moves made in the game along with their corresponding symbols.
     *
    ...
    /**
     * Sends a signal to reload the application, typically for updating the frontend.
     */
    void sendReloadSignal();

    /**
     * Removes duplicate entries from a list of moves based on the move's string representation.
     *
    ...
    /**
     * Retrieves the move list from the evaluation engine, including evaluations for each move.
     *
    ...
    /**
     * Checks the game state to confirm it aligns with expectations for the given engine.
     *
    ...
    /**
     * Determines if a human player is allowed to interact with the game at the current state.
     *
     * @param chessGame The current game state.
