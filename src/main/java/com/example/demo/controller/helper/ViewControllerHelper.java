 ...
 * the backend chess logic and the frontend view, ensuring that the visual elements
 * remain in sync with the actual game state.
 *
 * Core responsibilities include:
 * - Adding attributes to the model for rendering the chessboard and UI elements.
 * - Creating new fields and pieces for display based on game state changes.
 * - Configuring engine settings and managing engine shutdown hooks.
 * - Setting up game clocks with specified increments and handling timeout actions.
 * - Saving and updating game states to support resumable gameplay and adjustments to
 *   engine parameters for improved performance and player experience.
 *
 * This interface acts as a bridge, maintaining a separation between game logic and
 * UI configuration to enable a responsive and interactive user experience.
 */
public interface ViewControllerHelper {
    ...
    /**
     * Creates new fields representing the chessboard squares, setting up
     * attributes such as color and coordinates for display.
     */
    ...
    /**
     * Creates new display representations for chess pieces based on their
     * current positions in the given Game instance.
     *
    ...
    /**
     * Updates settings specific to UCI engines, such as depth, threads,
     * hash size, and move overhead, for both white and black players.
     *
     ...
     * @param playerEngines          A map of available player engines to apply settings.
     */
    void updateUciEngineSettings(int uciEngineDepthForWhite, int threadsForWhite, int hashSizeForWhite,
                                 int contemptForWhite, int moveOverheadForWhite, int uciEloForWhite,
                                 int uciEngineDepthForBlack, int threadsForBlack, int hashSizeForBlack,
                                 int contemptForBlack, int moveOverheadForBlack, int uciEloForBlack,
                                 String selectedEngineForWhite, String selectedEngineForBlack,
